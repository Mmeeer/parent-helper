<template>
  <div>
    <PageHeader title="Alerts" subtitle="Platform-wide alert monitoring" :breadcrumbs="[{ label: 'Alerts' }]">
      <template #actions>
        <select v-model="typeFilter" @change="alertPage = 1; loadAlerts()" class="px-3 py-2 border border-gray-200 rounded-lg text-xs bg-white focus:ring-1 focus:ring-primary-500 outline-none">
          <option value="">All Types</option>
          <option v-for="t in alertTypes" :key="t" :value="t">{{ fmt.formatAlertType(t) }}</option>
        </select>
        <select v-model="readFilter" @change="alertPage = 1; loadAlerts()" class="px-3 py-2 border border-gray-200 rounded-lg text-xs bg-white focus:ring-1 focus:ring-primary-500 outline-none">
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <h2 class="text-sm font-semibold text-gray-800 mb-4">Alert Trend (14 days)</h2>
          <div v-if="summary?.trend?.length" class="flex items-end gap-1 h-32">
            <div v-for="day in summary.trend" :key="day.date" class="flex-1 flex flex-col items-center gap-1">
              ...
              <div class="w-full bg-yellow-400 rounded-t" :style="{ height: barHeight(day.count) + '%', minHeight: day.count ? '4px' : '1px' }"></div>
              <span class="text-[9px] text-gray-400 -rotate-45 origin-left whitespace-nowrap">{{ day.date.slice(5) }}</span>
            </div>
          </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <h2 class="text-sm font-semibold text-gray-800 mb-4">By Type (14 days)</h2>
          <div v-if="Object.keys(summary?.byType ?? {}).length" class="space-y-2.5">
            <div v-for="(count, type) in summary.byType" :key="type">
              ...
              <div class="w-full bg-gray-100 rounded-full h-1.5">
                <div class="h-1.5 rounded-full bg-yellow-400" :style="{ width: `${Math.min(100, (count as number / maxTypeCount) * 100)}%` }"></div>
              </div>
            </div>
          </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <table v-else-if="alerts.length" class="w-full">
          <thead>
            <tr class="bg-gray-50/80 border-b border-gray-100">
              ...
              <th class="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase">Parent</th>
              <th class="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase">Message</th>
              <th class="px-4 py-2.5 text-left text-[11px] font-semibold text-gray-400 uppercase">Read</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-50">
            <tr v-for="a in alerts" :key="a._id" class="text-xs hover:bg-gray-50/50 transition">
              <td class="px-4 py-2.5 text-gray-400 whitespace-nowrap">{{ fmt.formatDate(a.createdAt, 'relative') }}</td>
              ...
              <td class="px-4 py-2.5 text-gray-600">{{ a.parentId?.name || a.parentId?.email || '-' }}</td>
              <td class="px-4 py-2.5 text-gray-500 truncate max-w-xs">{{ a.message }}</td>
              <td class="px-4 py-2.5">
                <span class="w-2 h-2 rounded-full inline-block" :class="a.read ? 'bg-gray-300' : 'bg-blue-500'"></span>
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <!-- Pagination -->
        <div v-if="alertTotalPages > 1" class="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/50">
          <p class="text-xs text-gray-400">Page {{ alertPage }} of {{ alertTotalPages }} ({{ alertTotal }} total)</p>
