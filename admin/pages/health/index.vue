<template>
  <div>
    <PageHeader title="System Health" subtitle="Real-time monitoring and system metrics" :breadcrumbs="[{ label: 'Health' }]">
      <template #actions>
        <button @click="loadHealth"
          class="px-3 py-2 text-xs font-medium text-white bg-ink-900 rounded-lg hover:bg-ink-800 transition">
          Refresh
        </button>
      </template>
    </PageHeader>
    ...
    <template v-else-if="health">
      <!-- Status Banner -->
      <div class="rounded-xl p-4 mb-6 flex items-center gap-3 border"
        :class="allHealthy ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'">
        <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" :class="allHealthy ? 'bg-green-100' : 'bg-yellow-100'">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard label="Devices Online" :value="health.devices.online" :subtitle="`of ${health.devices.total} paired`" color="green"
          icon='<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>' />
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl border border-ink-100 p-5">
          <h2 class="text-sm font-semibold text-ink-800 mb-4">Device Status</h2>
          <div class="flex items-center gap-6">
            <div class="relative w-28 h-28 flex-shrink-0">
              <svg viewBox="0 0 36 36" class="w-28 h-28 transform -rotate-90">
                <circle cx="18" cy="18" r="15.9" fill="none" stroke="#e7e5e4" stroke-width="3" />
            ...
            <div class="space-y-2.5 text-xs">
              <div class="flex items-center gap-2">
                ...
                <span class="text-ink-600">Online: {{ health.devices.online }}</span>
              </div>
              <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full bg-red-400 flex-shrink-0"></div>
                <span class="text-ink-600">Offline: {{ health.devices.offline }}</span>
              </div>
              <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full bg-ink-300 flex-shrink-0"></div>
                <span class="text-ink-600">Total: {{ health.devices.total }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Alert Distribution -->
        <div class="bg-white rounded-xl border border-ink-100 p-5">
          <h2 class="text-sm font-semibold text-ink-800 mb-4">Alerts This Week (by type)</h2>
      ...
      <!-- Server Info -->
      <div class="bg-white rounded-xl border border-ink-100 p-5">
        <h2 class="text-sm font-semibold text-ink-800 mb-4">Server Information</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
          <div><span class="text-ink-400">Server Time</span><div class="font-semibold text-ink-700 mt-0.5">{{ fmt.formatTime(health.serverTime) }}</div></div>
          <div><span class="text-ink-400">Uptime</span><div class="font-semibold text-ink-700 mt-0.5">{{ fmt.formatUptime(health.uptime) }}</div></div>
          <div><span class="text-ink-400">New Users This Week</span><div class="font-semibold text-ink-700 mt-0.5">{{ health.users.newThisWeek }}</div></div>
        </div>
      </div>
    </template>
  </div>
